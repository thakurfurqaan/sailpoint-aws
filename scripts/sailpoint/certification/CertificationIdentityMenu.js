...
SailPoint.CertificationIdentityMenu = function(element, config) {
...
Ext.apply(Ext.apply(SailPoint.CertificationIdentityMenu.prototype, SailPoint.BaseMenu.prototype), {
    ...
    createMenu: function(menu) {
        ...
        if (this.hasComments) {
            ...
            var commentsPanel = Ext.get(commentsPanelId);
            var action = (commentsPanel && commentsPanel.isVisible()) ? "Hide" : "View";
    ...
    viewIdentityWorkItem: function() {
      Ext.getDom('editForm:selectedWorkItemId').value = this.workItemId;
      try { Ext.getDom('editForm:viewWorkItemButton').click(); } catch(e) {}
    },

    viewIdentityCompletionComments: function() {
        showHideWithLock(Ext.getDom("delegationComments_" + this.delegationId));
    }
});
