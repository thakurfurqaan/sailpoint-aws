...
SailPoint.DateTimeInputConstructor = function(config) {
    ...
    var spDateInput = config.inputEl,
        date = getDateFromInput(spDateInput),
        uniqueDatePanelId = getUniqueId(config.renderTo);
    ...
    config.items = [{
        name: 'DatePanel',
        /* bug26312 :: an id was added in bug18814 to handle clearing date fields, I'm making it unique to avoid
         * collisions on panels where multiple date fields are shown.
         */
        id: uniqueDatePanelId,
        xtype: 'panel',
        hideLabel: true,
        border: false,
        layout: 'fit',
            ...
            listeners: {
                'change' : {
                    fn: function(timeField, newValue, oldValue) {
                        var dp = Ext.getCmp(uniqueDatePanelId);
...
function getUniqueId(dateFieldId) {
    return dateFieldId.substr(dateFieldId.indexOf("_"));
}
