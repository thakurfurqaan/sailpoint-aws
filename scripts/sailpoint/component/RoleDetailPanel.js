...
Ext.define('SailPoint.role.ClassificationsPanelTab', {
    extend: 'Ext.panel.Panel',

    constructor: function(config) {
        var store = SailPoint.Store.createRestStore({
            storeId: 'classificationGridStore',
            url: SailPoint.getRelativeUrl('/rest/identities/' + config.identityId + '/identityEntitlementRoles/' +
                    config.roleId + '/classifications'),
            autoLoad: false, // defer loading until the tab is clicked on
            remoteSort: true,
        ...
        var columns = [{
               ...
               header : '#{msgs.classifications_tbl_header_name}',
               sortable : true
           },{
               name : 'description',
               dataIndex : 'description',
               header : '#{msgs.classifications_tbl_header_desc}'
           },{
               name : 'origin',
               dataIndex : 'origin',
               header : '#{msgs.classifications_tbl_header_origin}',
               sortable: true
           }
        ]
        ...
        Ext.apply(config, {
            // Apply a listener to load the rest store only when this tab is shown
            listeners: {
                show: {
                    fn: function() { 
                        if (grid && grid.store) {
...
Ext.define('SailPoint.role.DetailsPanel', {
	...
	constructor: function(config) {
      ...
      windowType : config.windowType,
        ...
        if (config.hasClassifications) {
            var classificationsPanel = Ext.create('SailPoint.role.ClassificationsPanelTab', {
                title: '#{msgs.role_detail_tab_classifications}',
                roleId: config.roleId,
                assignmentId: config.assignmentId,
                identityId: config.identityId
            });

            config.items.push(classificationsPanel);
        }
...
SailPoint.RoleDetailPanel.window = function(assignmentId, selectedRoleId, selectedIdentityId, isAssignedRole, windowType, roleType, requestId, id2, refererType, refererId, hasClassifications) {
