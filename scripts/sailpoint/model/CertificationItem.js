...
Ext.define('SailPoint.CertificationItem', {
    ...
    constructor: function(record, gridId){
        ...
        if (decisionOptions.decisions){
            var statuses = [];
            Ext.each(decisionOptions.decisions, function(decisionObj){
    ...
    getCurrentStatus : function(){
        return this.record.get('IIQ_decisionChoices').currentState.name;
    },
    ...
    isAllowAcknowledgement : function(){
        ...
        if (actionList){
            Ext.each(actionList, function(item){
    ...
    isStatusAllowed : function(status){
        ...
        var found = false;
        Ext.each(this.allowedStatuses, function(val){
            if (val == status)
                found = true;
        });
