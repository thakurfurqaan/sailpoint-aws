Ext.define('SailPoint.HtmlEditor', {
  ...
  cleanHtml: function(text) {
    ...
    Ext.each(removals, function(s){
      text = Ext.String.htmlDecode(text).replace(s, "");
    });
    
    //IIQETN-6422: For IE, replacing <p> with <br> or ""
    // The following lines unify the behavior between browsers
    if(text.indexOf("<p>") === 0 || text.indexOf("<P>") === 0 ){
        text = text.replace(/<p>/i, "");
    }
    text = text.replace(/<\/p>/ig, "");
    text = text.replace(/<p>/ig, "<br>");
    text = text.replace(/<br><\/div>/ig, "<br>");
