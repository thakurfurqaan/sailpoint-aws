...
Ext.define('SailPoint.form.ComboBox', {
    ...
    constructor: function(config) {
        ...
        // This might already be set, for example from a MultiSelect.
        if (!config.listConfig.getInnerTpl) {
            config.listConfig.getInnerTpl = function(displayField) {
                // Default tpl expression encodes the displayField value to avoid XSS throughout
                // combo results, consumers should set comboTemplate if they want HTML.
                // I dont anticipate anyone expecting a combo result to contain renderable HTML.
                return config.comboTemplate || '{' + displayField + ':htmlEncode}';
            };
        }
