  ...
  var triggersGrid = new SailPoint.grid.PagingGrid({
    ...
    tbar: [
      ...
      ' ',
      {
        ...
        cls : 'primaryBtn',
        handler: function() {
          Ext.getDom("editForm:newTriggerButton").click();
        }
      }
    ]
  });
...
SailPoint.Define.Trigger.Triggers.editTrigger = function(triggerId) {
    Ext.getDom('editForm:selectedId').value = triggerId;
    Ext.getDom('editForm:editTriggerButton').click();
};
...
SailPoint.Define.Trigger.Triggers.deleteTrigger = function(btn, text) {
    if(btn=='yes') {
      Ext.getDom('editForm:deleteTriggerButton').click();
    }
};
