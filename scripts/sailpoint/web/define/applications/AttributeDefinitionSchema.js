 ...
 * @param uuid Uid for the DTO used by the schema attribute definition
 *
 */
SailPoint.define.applications.createAttributeDefinitionSchema = function(uuid) {
    // JSON object stored as string in this input, decode it and use it to create the SchemaAttributeDefinition
    var config = Ext.decode(Ext.getDom('attributesJSON_' + uuid).value);
...
SailPoint.define.applications.computeWindowTitle = function(attributeName) {
    ...
    if(!Ext.isEmpty(attributeName)) {
        return Ext.String.htmlEncode(attributeName) + ' - #{msgs.attr_def_schema_popup_title}';
    } else {
