...
Page.addEvents(
    ...
    /**
     * @event checkEditButton
     * Fired to check and see if the edit button for the correlation wizard should
     * be enabled or not (if the user has selected a valid correlation config)
     */
    'checkEditButton'
);

Page.on('checkEditButton', function() {
    // IIQSR-200 check if edit button needs to be enabled or disabled
    var configItem = Ext.get('editForm:currentConfigSelection');
    var configIndex = configItem.dom.selectedIndex;
    var wizButton = Ext.get('editWizardButton');
    if ( configIndex == 0) {
        wizButton.dom.disabled = "true";
    } else {
        if (wizButton.dom.hasAttribute("disabled")) {
            wizButton.dom.removeAttribute("disabled");
        }
    }
});
...
function showExtTab(tab) {
    ...
    if (tabId == 'schemaContent') {
        ...
        Page.fireEvent('checkEditButton');
