...
SailPoint.Web.Define.Policy.SimulationWindows = {
                                    ...
                                    fn : function (button) {
                                        if (button === 'ok') {
                                            if (isPolicy === true && Ext.get('simulatePolicyButton') && Ext.get('resultPolicyButton')) {
                                                ...
                                                Ext.get('resultPolicyButton').setVisibilityMode(Ext.Element.DISPLAY).hide();
                                            } else if (Ext.get('startSimulateRuleButton') && Ext.get('viewRuleResultsButton')) {
                                                Ext.get('startSimulateRuleButton').show();
                                                Ext.get('viewRuleResultsButton').setVisibilityMode(Ext.Element.DISPLAY).hide();
                                            } else {
            ...
            buttons: [{
                ...
                handler: function () {
                    ...
                    if (isPolicy === true) {
                        ...
                        var sel = Ext.getDom('mainForm:policyStateOption');
                        if(sel) {
                      ...
                      //Display the confirmation message depend on the state of the rule and then call the simulate action
                        if(isConstraintDisabled === null) {
                            var checkBox = Ext.getDom('mainForm:constraintDisabled');
                            if(checkBox) {
    ...
    simulateFromResultDialog: function (isPolicy, objID, simButton){
        if (Ext.getDom('mainForm:gridObjectId')) {
            Ext.getDom('mainForm:gridObjectId').value = objID;
        }
        if (Ext.getDom('mainForm:isPolicy')) {
            Ext.getDom('mainForm:isPolicy').value = isPolicy;
        }
        if (Ext.getDom(simButton)) {
            var bgMsg;
            if (Ext.getDom(simButton).id === "mainForm:simHidPolicyButton") {
                bgMsg = '#{msgs.msgbox_policy_simulation_text_in_background}';
                } else if (Ext.getDom(simButton).id === "mainForm:simulateRuleButton") {
                    bgMsg = '#{msgs.msgbox_rule_simulation_text_in_background}';
                } else {
                    Ext.getDom(simButton).click();
                }
            if (bgMsg && bgMsg !== "") {
                Ext.Msg.show({
                    msg: bgMsg,
    ...
    simulateFromRuleGrid: function (record){
        Ext.getDom('mainForm:gridObjectId').value = record.getId();
        Ext.getDom('mainForm:simulateButton').click();
    ...
    simulatePolicy: function (button, text) {
        var bgMsg = "";
        if (Ext.getDom('mainForm:isPolicy').value === "true") {
            bgMsg = '#{msgs.msgbox_policy_simulation_text_in_background}';
        } else {
            bgMsg = '#{msgs.msgbox_rule_simulation_text_in_background}';
        }
                ...
                fn : function (button) {
                    if (button === 'ok') {
                        if (Ext.getDom('mainForm:isPolicy').value === "true") {
                          ...
                          //edit page
                            Ext.getDom('mainForm:simHidPolicyButton').click();
                        } else {
                            //this indicates action is from individual rule
                            //edit page
                            Ext.getDom('mainForm:simulateRuleButton').click();
                        }
                    }
                }
            });
    ...
    setIsPolicy: function () {
        if (Ext.getDom('mainForm:isPolicy'))
            Ext.getDom('mainForm:isPolicy').value = "true";
        }
    };
