...
SailPoint.Role.Viewer.getTreePanel = function (config) {
    ...
    var treePanel = {
        ...
        listeners : {
            ...
            itemappend : {
                fn : function(aaa, node, index, eOpts) {
                    if ( !node.data.inScope) {
                        node.data.cls="outOfScopeNodeCls";
                    }
...
SailPoint.Role.Viewer.showDeletionResults = function() {
    ...
    if (deleteSucceeded) {
        ...
        // If we're currently displaying a tree view then we need to load it again.
        if (viewerState.currentView == SailPoint.Role.Viewer.ViewerModes.TOP_DOWN ||
                viewerState.currentView == SailPoint.Role.Viewer.ViewerModes.BOTTOM_UP) {
