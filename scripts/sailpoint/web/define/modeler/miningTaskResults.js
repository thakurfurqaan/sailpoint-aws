...
Ext.define('SailPoint.Role.Mining.TaskResultViewer', {
    ...
    refreshTaskResults: function() {
        Ext.getDom('TaskResultId').value = this.resultsToShow;
        Ext.getDom('resultsViewingForm:refreshTaskResults').click();
    },
    ...
    finishShowTaskResults: function() {
        ...
        var buttonsToHide = Ext.DomQuery.select('input[type=button]', Ext.getDom('taskResultsDetailsDiv'));
        var i;
        var buttonToHide;
        var body;
        var rowToHide;
        ...
        if (Ext.get('resultsViewingForm:report')) {
            ...
            body = Ext.dom.Query.selectNode('table > tbody', Ext.get('resultsViewingForm:report'));
