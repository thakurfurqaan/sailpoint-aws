...
SailPoint.Role.Tabs.getRoleTabPanel = function(config) {
    var userRights = Ext.JSON.decode(Ext.getDom('userRights').innerHTML);
    var tabPanels = [];
    var activePanel = Ext.getDom('tabState:activeTab').value;
    ...
    roleTabPanel.items.get(1).on('tabchange', function(tabPanel, activePanel) {
        ...
        Ext.getDom('tabState:tabPanelId').value = 'roleTabPanel';
        Ext.getDom('tabState:activeTab').value = activePanelId;
        // This form has been polluted with a bunch of grid config crap, 
        // so let's do a lighter-weight rest update
        // Ext.getDom('tabState:updateTabState').click();
        Ext.Ajax.request({
            ...
            callback: function() { 
                ...
                Ext.getDom('spErrorMsgsDiv').innerHTML = '';
...
SailPoint.Role.Tabs.createNew = function(item) {
    ...
    var miningPanel;
    var canEditRole = Ext.getDom('roleViewerRight').innerHTML == 'ManageRole';
    ...
    if (Ext.getDom('createNew')) {
        Ext.getDom('createNew').selectedIndex = 0;
    }
};
