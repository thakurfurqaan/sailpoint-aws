...
SailPoint.Analyze.initTabPanel = function() {
    var tabPanelState = JSON.parse(Ext.getDom('whereAreWe').innerHTML);
...
SailPoint.Analyze.Tabs.getAnalyzeTabPanel = function(config) {
    ...
    //Create combo Store from TabPanels
    var comboStore = SailPoint.Analyze.Tabs.getComboStore(tabPanels);
    var analyzeTabPanel = SailPoint.getMessagedTabPanel({
        ...
        tabBar: {
            items: [
                {
                    xtype: 'combo',
                    id: 'advAnalyticsSelector',
                    editable: false,
                    fieldLabel: '#{msgs.search_type}',
                    labelSeparator: '',
                    labelCls: 'spContentTitle',
                    store: comboStore,
                    ...
                    listeners: {
                        change: {
                            fn: function(combo, newVal, oldVal, opts) {
                                if (Ext.getCmp('analyzeTabPanel')) {
                                    Ext.getCmp('analyzeTabPanel').setActiveTab(newVal);
                                }
                            }
                        }
                    }
            ...
            listeners: {
                add: {
                    fn: function(tabBar, component, index, opts) {
                        if (component.getXType() === 'tab') {
    ...
    analyzeTabPanel.items.get(1).on('tabchange', function(tabPanel, activePanel) {
        if (activePanel) {
            Ext.getDom('stateForm:currentSearchPanel').value = activePanel.getId();
        }
...
SailPoint.Analyze.Tabs.getComboStore = function(panels) {
    ...
    var tabComboStore = Ext.create('Ext.data.Store', {
        model: 'SearchPanel'
    });

    var panelSize = panels.length;
    for (var i = 0; i< panelSize; i++) {
        tabComboStore.add(Ext.create('SearchPanel', {
            displayName: panels[i].title,
            tabName: panels[i].id
        }));
    }

    return tabComboStore;
}

SailPoint.Analyze.Tabs.getTabPanels = function() {
    var config;
    var tabPanels = [];
    var accessiblePanels = JSON.parse(Ext.getDom('accessiblePanels').innerHTML);
    var tabPanelState = JSON.parse(Ext.getDom('whereAreWe').innerHTML);

    if (accessiblePanels.identitySearch) {
        config = SailPoint.Analyze.getSearchPanelConfig('identitySearchPanel', '#{msgs.identity_search_combo_title}', tabPanelState);
    ...
    if (accessiblePanels.roleSearch) {
        config = SailPoint.Analyze.getSearchPanelConfig('roleSearchPanel', '#{msgs.role_search_combo}', tabPanelState);
    ...
    if (accessiblePanels.processInstrumentationSearch) {
        config = SailPoint.Analyze.getSearchPanelConfig('processInstrumentationSearchPanel', '#{msgs.process_instrumentation_search_combo}', tabPanelState);
