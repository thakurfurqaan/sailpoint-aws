...
SailPoint.Manage.Grid.Certifications.viewCertification = function(certId) {
    Ext.getDom('editForm:currentObjectId').value = certId;
...
SailPoint.Manage.Grid.Certifications.contextMenu = function(view, record, item, index, e, eOpts) {
    ...
    if (Ext.isEmpty(record.data.signed)) {
        contextMenu.add(
            {xtype: 'menuitem', text: '#{msgs.cert_menu_action_forward}', handler:  Ext.bind(SailPoint.Manage.Grid.Certifications.forwardFromMenu, null, [record.getId(), record.get("limitReassignments")]), iconCls: 'forwardBtn'}
        );
    }
