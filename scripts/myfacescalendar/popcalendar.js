...
org_apache_myfaces_CalendarInitData = function()
{
    ...
    this.todayString = "#{msgs.pop_calendar_today_is}";
    this.todayDateFormat = null;
    this.weekString = "#{msgs.pop_calendar_week_abbr}";
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//
// FUNCTIONS ADDED BY SAILPOINT
//
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

/**
 * Register a callback function that will be called when the calendar is shown.
 * 
 * @param {Function} fn  The function to call when the calendar is shown.
 */
org_apache_myfaces_PopupCalendar.prototype.onShow = function(fn) {
    this.onShowCallback = fn;
};

/**
 * Register a callback function that will be called when the calendar is hidden.
 * 
 * @param {Function} fn  The function to call when the calendar is hidden.
 */
org_apache_myfaces_PopupCalendar.prototype.onHide = function(fn) {
    this.onHideCallback = fn;
};

//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
//
// END FUNCTIONS ADDED BY SAILPOINT
//
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
org_apache_myfaces_PopupCalendar.prototype._showPopupPostProcess = function(overDiv)
{
    ...
    if (this.ie)
    {
        ...
        if (iframe == null)
        {
            ...
            Ext.get(iframe).setStyle('visibility', 'hidden').setStyle('position', 'absolute').setStyle('top', '0px').setStyle('left', '0px');
...
org_apache_myfaces_PopupCalendar.prototype._recalculateElement = function(overDiv)
{
    if (this.ie)
    {
        ...
        if (iframe)
        {
            ...
            // bug #17229 (and 18030): adding 19000 to default z-index of 999 to get around popup hidden under ext window
            popup.style.zIndex = 19999;
            ...
            // Added absolute here to prevent the iframe from appearing at the bottom 
            // of the browser in IE10.  See bug 17510 --Bernie
...
org_apache_myfaces_PopupCalendar.prototype.init = function(containerCtl)
{
    if (this.dom)
    {
        ...
        if (!this.calendarDiv)
        {
            ...
            Ext.get(this.calendarDiv).on('click', function()
            {
            ...
            Ext.get(mainTable).setStyle('width', ((this.initData.showWeekNumber == 1) ? 250 : 220) + "px;");
            ...
            Ext.get(contentTable).setStyle('width', ((this.initData.showWeekNumber == 1)?248:218) + "px;");
            ...
            Ext.get(document).on("keypress", this._keypresshandler.bind(this), false);
            Ext.get(document).on("click", this._clickhandler.bind(this), false);
        }
    }
    ...
    if (!this.ns4)
    {
        ...
        if (this.initData.showToday == 1)
        {
            ...
            Ext.get(todayLink).on("click", function(event)
            {
            ...
            Ext.get(todayLink).on("mousemove", function()
            {
                window.status = this.initData.gotoString;
            }.bind(this), false);
            Ext.get(todayLink).on("mouseout", function()
            {
                window.status = "";
            }.bind(this), false);
        ...
        Ext.get(this.monthSpan).on("mouseover", function(event)
        {
        ...
        Ext.get(this.monthSpan).on("mouseout", function(event)
        {
        ...
        Ext.get(this.monthSpan).on("click", function(event)
        {
            this._popUpMonth();
            event.stopPropagation();
        }.bind(this), false);
        ...
        Ext.get(this.yearSpan).on("mouseover", function(event)
        {
        ...
        Ext.get(this.yearSpan).on("mouseout", function(event)
        {
        ...
        Ext.get(this.yearSpan).on("click", function(event)
        {
            this._popUpYear();
            event.stopPropagation();
        }.bind(this), false);
...
org_apache_myfaces_PopupCalendar.prototype._appendNavToCaption = function(direction)
{
    ...
    Ext.get(imgLeft).setStyle("border", "0px");
...
org_apache_myfaces_PopupCalendar.prototype._createControl = function(direction, spanLeft, imgLeft)
{
    ...
    Ext.get(spanLeft).on("mouseover", function(event)
    {
    ...
    Ext.get(spanLeft).on("click", function()
    {
        if (direction == "left")
        {
            this._decMonth();
    ...
    Ext.get(spanLeft).on("mouseout", function(event)
    {
    ...
    Ext.get(spanLeft).on("mousedown", function()
    {
    ...
    Ext.get(spanLeft).on("mouseup", function()
    {
        clearTimeout(this.timeoutID1);
        clearInterval(this.intervalID1);
    }.bind(this), false);
...
org_apache_myfaces_PopupCalendar.prototype._constructMonth = function()
{
    ...
    if (!this.monthConstructed)
    {
        ...
        Ext.get(selectMonthTable).setStyle("width", "70px").setStyle("border-collapse", "collapse");
        ...
        Ext.get(selectMonthTable).on("mouseover", function()
        {
            clearTimeout(this.timeoutID1);
        }.bind(this), false);
        Ext.get(selectMonthTable).on("mouseout", function(event)
        {
            clearTimeout(this.timeoutID1);
            this.timeoutID1 = setTimeout((function()
            {
        ...
        for (i = 0; i < 12; i++)
        {
            ...
            if (i == this.selectedDate.month)
            {
                ...
                Ext.get(sNameNode).setStyle("font-weight", "bold");
            ...
            Ext.get(monthCell).on("mouseover", function(event, element)
            {
                Ext.getDom(element).className = this.initData.themePrefix + "-dropdown-select-style";
            }.bind(this), false);

            Ext.get(monthCell).on("mouseout", function(event, element)
            {
                Ext.getDom(element).className = this.initData.themePrefix + "-dropdown-normal-style";
            }.bind(this), false);

            Ext.get(monthCell).on("click", function(event, element)
            {
                this.monthConstructed = false;
                this.selectedDate.month = parseInt(Ext.getDom(element).getAttribute("userData"),10);
                this._constructCalendar();
                this._popDownMonth();
                event.stopPropagation();
            }.bind(this), false);
...
org_apache_myfaces_PopupCalendar.prototype._constructYear = function()
{
    ...
    if (!this.yearConstructed)
    {
        ...
        Ext.get(selectYearTable).setStyle("width", "44px").setStyle("border-collapse", "collapse");
        ...
        Ext.get(selectYearTable).on("mouseover", function()
        {
            clearTimeout(this.timeoutID2);
        }.bind(this), false);
        Ext.get(selectYearTable).on("mouseout", function(event)
        {
            clearTimeout(this.timeoutID2);
        ...
        Ext.get(selectYearCellMinus).on("mouseover", function(event, element)
        {
            Ext.getDom(element).className = this.initData.themePrefix + "-dropdown-select-style";
        }.bind(this), false);

        Ext.get(selectYearCellMinus).on("mouseout", function(event, element)
        {
            clearInterval(this.intervalID1);
            Ext.getDom(element).className = this.initData.themePrefix + "-dropdown-normal-style";
        }.bind(this), false);

        Ext.get(selectYearCellMinus).on("mousedown", function(event, element)
        {
            clearInterval(this.intervalID1);
            this.intervalID1 = setInterval((function()
            {
        ...
        Ext.get(selectYearCellMinus).on("mouseup", function(event)
        {
            clearInterval(this.intervalID1);
            event.stopPropagation();
        }.bind(this), false);
        ...
        for (i = this.selectedDate.year - 3; i <= (this.selectedDate.year + 3); i++)
        {
            ...
            Ext.get(yearCell).on("mouseover", function(event, element)
            {
                Ext.getDom(element).className = this.initData.themePrefix + "-dropdown-select-style";
            }.bind(this), false);

            Ext.get(yearCell).on("mouseout", function(event, element)
            {
                Ext.getDom(element).className = this.initData.themePrefix + "-dropdown-normal-style";
            }.bind(this), false);

            Ext.get(yearCell).on("click", function(event, element)
            {
                var elem = Ext.getDom(element);
                var sYear = null;
        ...
        Ext.get(selectYearCellPlus).on("mouseover", function(event, element)
        {
            Ext.getDom(element).className = this.initData.themePrefix + "-dropdown-select-style";
        }.bind(this), false);

        Ext.get(selectYearCellPlus).on("mouseout", function(event, element)
        {
            clearInterval(this.intervalID2);
            Ext.getDom(element).className = this.initData.themePrefix + "-dropdown-normal-style";
        }.bind(this), false);

        Ext.get(selectYearCellPlus).on("mousedown", function(event, element)
        {
            clearInterval(this.intervalID2);
        ...
        Ext.get(selectYearCellPlus).on("mouseup", function(event)
        {
            clearInterval(this.intervalID2);
        }.bind(this), false);
...
org_apache_myfaces_PopupCalendar.prototype._appendCell = function(parentElement, value)
{
    ...
    Ext.get(cell).setStyle("text-align", "right");
    ...
    sNormalStyle, sSelectStyle)
{
    ...
    if (weekSelectable)
    {
        ...
        Ext.get(link).on("mousemove", function(event)
        {
        ...
        Ext.get(link).on("mouseout", function(event, element)
        {
            var elem = Ext.getDom(element);
        ...
        Ext.get(link).on("click", function(event, element)
        {
            var elem = Ext.getDom(element);
        ...
        Ext.get(link).on("mouseover", function(event, element)
        {
            var elem = Ext.getDom(element);
...
org_apache_myfaces_PopupCalendar.prototype._constructCalendar = function()
{
    ...
    var contentTable = document.createElement("table");
    Ext.get(contentTable).setStyle("border", "0px");
    ...
    if (this.initData.showWeekNumber == 1)
    {
        ...
        Ext.get(showWeekNumberCell).setStyle("width", "27px").setStyle("font-weight", "bold");
        ...
        Ext.get(dividerCell).setStyle("width", "1px");
        ...
        Ext.get(dividerImg).setStyle("width", "1px");
    ...
    for (i = 0; i < 7; i++)
    {
        ...
        Ext.get(dayNameCell).setStyle("width", "27px").setStyle("text-align", "right").setStyle("font-weight", "bold");
    ...
    for (datePointer = 1; datePointer <= numDaysInMonth; datePointer++)
    {
        ...
        Ext.get(dateCell).setStyle("text-align", "right");
        ...
        if (this._isDaySelectable())
        {
            ...
            Ext.get(dateLink).on("mousemove", function(event)
            {
            ...
            Ext.get(dateLink).on("mouseout", function(event, element)
            {
                var elem = Ext.getDom(element);
                elem.className = elem.sNormalStyle;
                window.status = "";
            }.bind(this), false);
            Ext.get(dateLink).on("click", function(event, element)
            {
                var elem = Ext.getDom(element);
            ...
            Ext.get(dateLink).on("mouseover", function(event, element)
            {
                var elem = Ext.getDom(element);
                elem.className = elem.sSelectStyle;
            }.bind(this), false);
    ...
    Ext.get(this.changeMonthImg).setStyle("border", "0px");
    ...
    Ext.get(this.changeYearImg).setStyle("border", "0px");
    ...
    if (this.acceptMonthSpan)
    {
      ...
      Ext.get(acceptMonthImg).setStyle("border", "0px");
    ...
    Ext.get(closeButtonImg).setStyle("border", "0px");
