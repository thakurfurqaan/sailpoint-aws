...
/*
 * Ext override for bug#24214
 * XSS filter for error message in Date field.
 */
Ext.override(Ext.form.DateField, {
    getErrors: function(value) {
        var errors = this.callParent(arguments);
        var escapedErrors = [];
        Ext.Array.forEach(errors, function(item){
          	escapedErrors.push(Ext.String.htmlEncode(item));
        });
...
/*
 * Ext override for bug#24214
 * XSS filter for error message in Number field.
 */
...
/**
 * Bug #18474
 * SUPERHACK: When using renderTo and modal together, it doesn't become a 'true' modal.
 * Basically, the mask is only on the div that the window is rendered to. To get around that, 
 * we will set the floatParent before the call toFront to return the body element so the
 * hack goes over the whole page.
 * 
 * See Ext.util.Floating.toFront and Ext.ZIndexManager._showModalMask
 */
Ext.override(Ext.window.Window, {
    /**
     * To skip the hack for modal masks and mask only the renderTo element,
     * set this to true
     */
    skipModalHack: false,
    ...
    toFront: function() {
        var me = this,
            hackModal = !me.skipModalHack && me.modal && me.renderTo,
            oldFloatParent;

        if (hackModal) {
            try {
                oldFloatParent = me.floatParent;
                me.floatParent = {
...
/*
 * Bug #25866
 * Overriding the labels on the paging toolbar so that they correspond to the 
 * message catalog
 */
Ext.override(Ext.toolbar.Paging, {
    getPagingItems: function() {
        ...
        return [{
            itemId: 'first',
            tooltip: '#{msgs.paging_first}',
            overflowText: '#{msgs.paging_first}',
            ...
            itemId: 'prev',
            tooltip: '#{msgs.paging_prev_text}',
            overflowText: '#{msgs.paging_prev_text}',
        ...
        '#{msgs.paging_before_text}',
        {
            ...
            text: Ext.String.format('#{msgs.paging_after_text}', 1)
        },
        '-',
        {
            itemId: 'next',
            tooltip: '#{msgs.paging_next_text}',
            overflowText: '#{msgs.paging_next_text}',
            ...
            itemId: 'last',
            tooltip: '#{msgs.paging_last}',
            overflowText: '#{msgs.paging_last}',
        ...
        {
            itemId: 'refresh',
            tooltip: '#{msgs.paging_refresh}',
            overflowText: '#{msgs.paging_refresh}',
